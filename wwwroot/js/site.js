...
function initGridSearch(inputId, cardSelector, emptyId) {
    ...
    input.addEventListener('input', function () {
        ...
        const pager = input.closest('.stats-wrapper')?.querySelector('[data-pager]');
        if (pager) {
            pager._goToPage(1);
        }
    });
...
function initGridPagination(cardSelector, pagerId, perPage = 9) {
    ...
    function renderPager(totalPages) {
        ...
        const mkItem = (label, page, disabled, active) => {
            ...
            a.href = '#';
            a.innerHTML = label;
            a.addEventListener('click', e => { e.preventDefault(); if (!disabled) goToPage(page); });
            li.appendChild(a);
            ul.appendChild(li);
        };
    ...
    // Init
    getCards().forEach(c => { if (c.dataset.searchVisible === undefined) c.dataset.searchVisible = '1'; });
    goToPage(1);
    container._goToPage = goToPage;
}
